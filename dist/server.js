...
io.on("connection", (socket) => {
    ...
    socket.on("sendGroupMessage", async ({ conversationId, userId, message: text }) => {
        const conversation = await GroupConversation.findOne({
            _id: conversationId,
        });
        if (!conversation)
            throw new AppError("no group conversation found for that id", 404);
        const messagesArray = conversation?.defaultLanguages?.map(async (language) => {
            const options = {
                method: "POST",
    ...
    socket.on("sendMessageChatGPT", async (data) => {
        ...
        if (toFront)
            io.to(selectUserSocket).emit("getMessage", data);
        ...
        if (!conversation) {
            ...
            User.findOneAndUpdate({ _id: to }, { $push: { conversations: conversation.id } });
        }
        let response;
        try {
        ...
        await Conversation.findOneAndUpdate({ users: { $all: [from, to] } }, { $push: { messages: messageReply.id } });
    });
