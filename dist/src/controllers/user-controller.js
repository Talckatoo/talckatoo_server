...
exports.getUsers = catchAsync(async (req, res, next) => {
    ...
    modifiedUsers.sort((a, b) => {
        if (a.conversation["updatedAt"].getTime() <
...
const getFriends = async (req, res, next) => {
    try {
        ...
        for (const friend of friends.friends) {
            ...
            const sharedConversation = friend.conversations.find((conversationId) => currentUser.conversations.includes(conversationId));
            const populateOptions = {
                path: "messages",
            ...
            let conversation = sharedConversation
                ? await Conversation.findById(sharedConversation).populate(populateOptions)
                : null;
            if (conversation) {
...
exports.getUserConversations = catchAsync(async (req, res, next) => {
    ...
    const conversations = await Conversation.find({ users: userId }).populate(populateOptions);
...
exports.updateProfile = catchAsync(async (req, res, next) => {
    ...
    if (!updateObj.profileImage.public_id)
        updateObj = {};
    if (userName)
...
exports.editUserConversation = catchAsync(async (req, res, next) => {
    ...
    let conversation = await Conversation.findOneAndUpdate({ _id: conversationId }, {
        $pull: { unread: userId },
